...
def test_http_server_with_auth_builds():
    settings = Settings(
        auth_enabled=True,
        auth_secret="x" * 32,
        # Pflicht seit der Publikums- und der Issuerbindung: ohne
        # `oauth_audience` und ohne `oauth_issuer` entsteht gar kein Verifier
        # mehr (siehe tests/test_security.py).
        oauth_audience="swiss-courts-mcp",
        oauth_issuer="https://idp.example.ch",
